...
public class HighScoreStore {
    ...
    public boolean qualifies(int score) {
        return entries.size() < MAX_ENTRIES || (!entries.isEmpty() && score > entries.get(entries.size() - 1).score())
                || entries.isEmpty();
    }

    public int addScore(String name, int score) {
        entries.add(new Entry(name.isBlank() ? "Pilot" : name, score));
        entries.sort((a, b) -> b.score() - a.score());
        while (entries.size() > MAX_ENTRIES) {
            entries.remove(entries.size() - 1);
        }
        save();
        return entries.indexOf(new Entry(name.isBlank() ? "Pilot" : name, score));
    }
}
